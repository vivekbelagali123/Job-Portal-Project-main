...
const JobDescription = () => {
    ...
    const [isApplied , setIsApplied] = useState(isInitiallyApplied);        // updating ui for job when user apply
    ...
    const applyJobHandler = async() => {
        try {
            ...
            if(res.data.success){
                ...
                dispatch(setSingleJob(updatedSingleJob));  // helps UI to update in real time 
        ...
        <div className='max-w-7xl mx-auto my-10'>
            <div className='flex items-center justify-between'>
                ...
                <Button
                    ...
                    disabled={isApplied}
                    className={`rounded-lg ${isApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#433bff] hover:bg-[#2f27ce]'}`}>
            ...
            <div className='my-4'>
                ...
                <h1 className='font-bold my-1'>Experience : <span className='pl-4 font-medium text-gray-800'>{singleJob?.experience} yrs</span></h1>
                <h1 className='font-bold my-1'>Salary : <span className='pl-4 font-medium text-gray-800'>{singleJob?.salary} LPA</span></h1>
                <h1 className='font-bold my-1'>Total Applicants : <span className='pl-4 font-medium text-gray-800'>{singleJob?.application.length}</span></h1>
                <h1 className='font-bold my-1'>Posted Date : <span className='pl-4 font-medium text-gray-800'>{singleJob?.createdAt.split("T")[0]}</span></h1>
